  ...
  async ({ bond, provider, networkID, address }: IChangeApproval, { dispatch }) => {
    ...
    try {
      ...
      if (bond === BONDS.DAI_CTXT) {
        approveTx = await reserveContract.approve(addresses.BONDS.DAI_CTXT, constants.MaxUint256);
      }
      dispatch(
...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, networkID }: ICalcBondDetails) => {
    ...
    const bondCalcContract = new ethers.Contract(addresses.CTXT_BONDING_CALC_ADDRESS, BondingCalcContract, provider);
    ...
    console.log('calcBondDetails', bond, value);

    console.log(1);
    const maiPrice = await getTokenPrice('DAI');
    console.log(2, maiPrice);
    ...
    if (bond === BONDS.DAI_CTXT) {
      valuation = await bondCalcContract.valuation(addresses.RESERVES.DAI_CTXT, amountInWei);
