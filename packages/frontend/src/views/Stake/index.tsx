...
function Stake() {
  ...
  const ContextBalance = useSelector<IReduxState, string>(state => {
    return state.account.balances && state.account.balances.ctxt;
  });
  const sCTXTBalance = useSelector<IReduxState, string>(state => {
    return state.account.balances && state.account.balances.sCtxt;
  });
  const stakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.ctxtStake;
  });
  const unstakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.sCtxtUnstake;
  });
  ...
  const setMax = () => {
    if (view === 0) {
      setQuantity(ContextBalance);
    } else {
      setQuantity(sCTXTBalance);
    }
  };
  ...
  const hasAllowance = useCallback(
    token => {
      if (token === 'Context') return stakeAllowance > 0;
      if (token === 'sCTXT') return unstakeAllowance > 0;
      return 0;
    },
    [stakeAllowance],
  );
  ...
  const trimmedSContextBalance = trim(Number(sCTXTBalance), 4);
  const stakingRebasePercentage = trim(stakingRebase * 100, 4);
  const nextRewardValue = trim((Number(stakingRebasePercentage) / 100) * Number(trimmedSContextBalance), 4);
          ...
          <Grid container direction="column" spacing={2}>
            <Grid item>
              <div className="card-header">
                <p className="single-stake-title">Context Staking (💄,💄)</p>
                <RebaseTimer />
              </div>
            </Grid>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="stake-index">
                      ...
                      <p className="single-stake-subtitle-value">
                        {currentIndex ? <>{trim(Number(currentIndex), 3)} sCTXT</> : <Skeleton width="150px" />}
                      </p>
                    </div>
                  </Grid>
                </Grid>
            ...
            <div className="staking-area">
              {!address ? (
                <div className="stake-wallet-notification">
                  ...
                  <p className="desc-text">Connect your wallet to stake Context tokens!</p>
                </div>
              ) : (
                <>
                  <Box className="stake-action-area">
                    ...
                    <Box className="stake-action-row" display="flex" alignItems="center">
                      ...
                      <TabPanel value={view} index={1} className="stake-tab-panel">
                        {address && hasAllowance('sCTXT') ? (
                          ...
                          >
                            <p>{txnButtonText(pendingTransactions, 'unstaking', 'Unstake Context')}</p>
                          </div>
                        ) : (
                          <div
                            className="stake-tab-panel-btn"
                    ...
                    <div className="help-text">
                      {address &&
                        ((!hasAllowance('Context') && view === 0) || (!hasAllowance('sCTXT') && view === 1)) && (
                  ...
                  <div className={`stake-user-data`}>
                    {Number(warmupBalance) > 0 && (
                      <div className="data-row">
                        ...
                        <p className="data-row-value">
                          {isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(warmupBalance), 4)} CTXT</>}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(ContextBalance), 4)} CTXT</>}
                      </p>
                    </div>
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? (
                          ...
                          <>{new Intl.NumberFormat('en-US').format(Number(trimmedSContextBalance))} sCTXT</>
                        )}
                      </p>
                    </div>
                    ...
                    <div className="data-row">
                      ...
                      <p className="data-row-value">
                        {isAppLoading ? <Skeleton width="80px" /> : <>{nextRewardValue} sCTXT</>}
                      </p>
                    </div>
