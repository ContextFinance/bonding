...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const [uri, setUri] = useState(chainID === Networks.FUJI ? getTestnetURI() : getMainnetURI());
  ...
  const _checkNetwork = (otherChainID: number): Boolean => {
    if (Number(otherChainID) !== Networks.MAINNET) {
      alert('Please switch your wallet to Avalanche network to use Context!');
    }

    if (chainID !== otherChainID) {
      console.warn('You are switching networks: ', otherChainID);
      if (otherChainID === Networks.MAINNET || otherChainID === Networks.FUJI) {
