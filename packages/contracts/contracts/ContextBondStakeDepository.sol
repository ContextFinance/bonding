...
import './interfaces/IContextBondingCalculator.sol';
...
contract ContextBondStakeDepository is Ownable {
    ...
    address public immutable CTXT; // intermediate reward token from treasury
    address public immutable sCTXT; // token given as payment for bond
    ...
    struct Bond {
        ...
        uint256 gonsPayout; // sCTXT gons remaining to be paid
    }
    ...
    function deposit(
        ...
        uint256 value = IContextTreasury(treasury).valueOfToken(
            principle,
            _amount
        );
        ...
        IContextTreasury(treasury).deposit(_amount, principle, profit);

        if (fee != 0) {
            // fee is transferred to dao
            IERC20(CTXT).safeTransfer(DAO, fee);
        }

        // total debt is increased
        totalDebt = totalDebt.add(value);

        IERC20(CTXT).approve(staking, payout);
        IContextStaking(staking).stake(payout, address(this));
        IContextStaking(staking).claim(address(this));

        uint256 stakeGons = IsCTXT(sCTXT).gonsForBalance(payout);
        ...
        returns (uint256)
    {
        ...
        uint256 _amount = IsCTXT(sCTXT).balanceForGons(info.gonsPayout);
        emit BondRedeemed(_recipient, _amount, 0); // emit bond data
        IERC20(sCTXT).transfer(_recipient, _amount); // pay user everything due
        return _amount;
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
                .mul(
                    IContextBondingCalculator(bondCalculator).markdown(
                        principle
                    )
                )
                .div(100);
        ...
        returns (uint256 pendingPayout_)
    {
        ...
        uint256 payout = IsCTXT(sCTXT).balanceForGons(
