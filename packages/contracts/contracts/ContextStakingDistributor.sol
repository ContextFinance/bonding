...
contract ContextStakingDistributor is Ownable {
    ...
    function distribute() external returns (bool) {
        if (nextEpochTime <= block.timestamp) {
            ...
            for (uint256 i = 0; i < info.length; i++) {
                if (info[i].rate > 0) {
                    IContextTreasury(treasury).mintRewards( // mint and send from treasury
                        info[i].recipient,
    ...
    function nextRewardAt(uint256 _rate) public view returns (uint256) {
        return IERC20(CTXT).totalSupply().mul(_rate).div(1000000);
    }
