...
contract ContextBondingCalculator is IContextBondingCalculator {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == CTXT) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return
            reserve.mul(2 * (10**IERC20(CTXT).decimals())).div(
                getTotalValue(_pair)
            );
    }
}
