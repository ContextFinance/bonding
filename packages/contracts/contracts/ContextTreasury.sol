...
import './interfaces/IContextTreasury.sol';
...
contract ContextTreasury is Ownable, IContextTreasury {
    ...
    address public sCTXT;
    uint256 public sCTXTQueue; // Delays change to sCTXT address
    ...
    constructor(
        ...
        isLiquidityToken[_CTXTDAI] = true;
        liquidityTokens.push(_CTXTDAI);
        bondCalculator[_CTXTDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for CTXT
        @param _amount uint
    ...
    function deposit(
        ...
        // mint CTXT needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(CTXT).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn CTXT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = valueOfToken(_token, _amount);
        ICTXTERC20(CTXT).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sCTXT).balanceOf(msg.sender); // Can only borrow against sCTXT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with CTXT
        @param _amount uint
     */
    function repayDebtWithCTXT(uint256 _amount) external {
        require(isDebtor[msg.sender], 'Not approved');

        ICTXTERC20(CTXT).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, CTXT, _amount, _amount);
    }
        ...
        override
    {
        ...
        IERC20Mintable(CTXT).mint(_recipient, _amount);
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(IERC20(CTXT).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns CTXT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match CTXT decimals
            value_ = _amount.mul(10**IERC20(CTXT).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SCTXT) {
            // 9
            sCTXTQueue = block.number.add(blocksNeededForQueue);
        } else return false;
