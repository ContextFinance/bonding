...
let oldTreasury = Treasury.attach(
  '0xab328Ca61599974b0f577d1F8AB0129f2842d765'
).connect(multisig)
let newTreasury = Treasury.attach(
  '0x8ce47D56EAa1299d3e06FF3E04637449fFb01C9C'
).connect(deployer)

const Migrator = await ethers.getContractFactory('ClamTokenMigrator')
let migrator = Migrator.attach(
  '0xDaa1f5036eC158fca9E5ce791ab3e213cD1c41df'
).connect(deployer)

const ERC20 = await ethers.getContractFactory('ContextERC20V2')
let clam = ERC20.attach(addresses.OLD_CTXT_ADDRESS)
let clam2 = ERC20.attach(addresses.CTXT_ADDRESS)
let mai = ERC20.attach(addresses.DAI_ADDRESS)

const StakedContextERC20V2 = await ethers.getContractFactory(
  'StakedContextERC20V2'
)
let sClam = StakedContextERC20V2.attach(addresses.OLD_SCTXT_ADDRESS).connect(
  deployer
)
let sClam2 = StakedContextERC20V2.attach(addresses.sCTXT_ADDRESS).connect(
  deployer
)
...
const Bond = await ethers.getContractFactory('ContextBondDepository')
let maiBond = Bond.attach('0x603A74Fd527b85E0A1e205517c1f24aC71f5C263').connect(
  deployer
)
let lpBond = Bond.attach('0x706587BD39322A6a78ddD5491cDbb783F8FD983E').connect(
  deployer
)
...
await newTreasury.toggle('0', addresses.BONDS.DAI, zeroAddress)
await newTreasury.toggle('4', addresses.BONDS.DAI_CTXT, zeroAddress)
