// @dev. This script will deploy v2 of Context. It will deploy the whole ecosystem.
...
async function main() {
  ...
  const maiAddr = oldContractAddresses.DAI_ADDRESS

  // Deploy CTXT v2
  const CTXT2 = await ethers.getContractFactory('ContextERC20V2')
  // const newClam = CTXT2.attach('0xC250e9987A032ACAC293d838726C511E6E1C029d')
  const newClam = await CTXT2.deploy()
  await newClam.deployTransaction.wait()
  console.log('CTXT deployed: ' + newClam.address)

  const ContextCirculatingSupply = await ethers.getContractFactory(
    'ContextCirculatingSupply'
  )
  // const ContextCirculatingSupply = ContextCirculatingSupply.attach(
  ...
  await (await ContextCirculatingSupply.initialize(newClam.address)).wait()
  ...
  // Deploy sCTXT
  const StakedCTXT = await ethers.getContractFactory('StakedContextERC20V2')
  ...
  console.log('sCTXTv2: ' + sCTXT.address)
  ...
  const migrator = await Migrator.deploy(
    oldContractAddresses.CTXT_ADDRESS,
  ...
  console.log(
    JSON.stringify({
      ...
      CTXT_ADDRESS: newClam.address,
      OLD_CTXT_ADDRESS: oldContractAddresses.CTXT_ADDRESS,
      OLD_SCTXT_ADDRESS: oldContractAddresses.sCTXT_ADDRESS,
      DAI_ADDRESS: maiAddr,
      ...
      RESERVES: {
        MAI: maiAddr,
        OLD_DAI_CTXT: oldContractAddresses.RESERVES.DAI_CTXT,
        DAI_CTXT: lpAddress,
      },
      BONDS: {
        OLD_MAI: oldContractAddresses.BONDS.DAI,
        MAI: maiBond.address,
        OLD_DAI_CTXT: oldContractAddresses.BONDS.DAI_CTXT_V2,
  ...
  // Set newTreasury for CTXT token
  ...
  await verify(sCTXT.address, [])
  await verify(ContextCirculatingSupply.address, [deployer.address])
  ...
  await verify(stakingWarmup.address, [staking.address, sCTXT.address])
